...
export function isMissingNodeError(error: unknown): boolean {
  ...
  if (typeof error === 'object') {
    ...
    if (code === 'ENOENT') {
      const syscall = 'syscall' in error ? (error as { syscall?: unknown }).syscall : undefined
      const pathValue = 'path' in error ? (error as { path?: unknown }).path : undefined
...
export function isMissingNodeMessage(message: string): boolean {
  ...
  if (/not recognized as an internal or external command/i.test(message) && /node/i.test(message)) return true
