...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    await dialog.getByRole('button', { name: '自动编辑' }).waitFor({ timeout: 10_000 })
  ...
  it('stores Permission as the default for future sessions without changing an existing session', async () => {
    ...
    const dialog = page.getByRole('dialog', { name: '设置' })
    await dialog.waitFor({ timeout: 10_000 })
    const selector = dialog.getByRole('button', { name: '自动编辑' })
    await selector.waitFor({ timeout: 10_000 })
    await expect.poll(() => selector.isEnabled(), { timeout: 5_000 }).toBe(true)
    await selector.click()
    await page.getByRole('menuitem', { name: '变更前确认' }).click()
    await dialog.getByRole('button', { name: '变更前确认' }).waitFor({ timeout: 10_000 })
    ...
    await dialog.getByRole('button', { name: '变更前确认' }).click()
