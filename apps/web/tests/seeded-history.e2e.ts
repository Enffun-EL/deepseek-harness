...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('an Access-chip switch lands one command row: bare name, non-repeating settlement text', async () => {
    ...
    await page.getByRole('button', { name: 'Access mode, current: Auto edit' }).click()
    await page.getByRole('menuitem', { name: 'Ask before edits' }).click()
    await page.getByRole('button', { name: 'Access mode, current: Ask before edits' }).waitFor({ timeout: 10_000 })
