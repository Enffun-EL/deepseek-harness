...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.workspaces` and `sidebar.settings` child slots, and injected `startSession`, sidebar-toggle, and `desktopRail` callbacks. There is no plugin store.

Plugin `Config.desktopRail` (default `false`) selects chrome without replacing the web path: when true, the `sidebar` slot mounts `DesktopRail` — a Zcode-like left-rail information architecture (New Task, Search, Automation, Plugin Market; project/task sections with empty copy; bottom account-cluster placeholders) that still declares the same child seats and reuses `startSession` for New Task. Web compositions leave the flag off and keep `SidebarRoot`. Desktop compositions set `config.desktopRail: true` on the ui-sidebar loader entry (or a future desktop slot that injects the same flag).

During a live collapse, the web shell holds the expanded content at its current width while it fades out for 150ms. The four upper controls—the shell toggle and New Session plus add and search rendered through `sidebar.workspaces`—then share one 150ms fade and 49px leftward translation into the 56px rail, ending with the layout's 300ms column slide; every 36px control box follows the same path to the rail's 10px left inset. The bottom-pinned `sidebar.settings` control shares the fade timing but has no horizontal translation. A page that starts collapsed renders the rail statically, and reduced-motion mode disables both transitions. DesktopRail uses the same column width and collapsed icon chrome without the web crossfade machine.
...
The foot is the `sidebar.settings` seat: the sidebar renders only the bottom-pinned layout slot and shares its column state (`wide`); ui-settings registers the trigger row and settings panel there. DesktopRail stacks account placeholders above that seat.

The `/client` exports are the plugin body (`apply`/`inject`/`Config`) plus the contract types only; SidebarRoot, DesktopRail, the row components, and the tree derivation remain package-internal behind the slot registration.
...
- **DesktopRail actions beyond New Task are presentational** — Search, Automation, Plugin Market, and account placeholders carry stable ids/actions for later routes; they do not navigate yet.
