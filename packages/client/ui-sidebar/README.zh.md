...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.workspaces` 与 `sidebar.settings` 子 slot，以及注入的 `startSession`、侧边栏切换与 `desktopRail` 回调。这里没有插件 store。

插件 `Config.desktopRail`（默认 `false`）在不替换 Web 路径的前提下选择外壳：为 true 时，`sidebar` slot 挂载 `DesktopRail`——Zcode 风格的左侧栏信息架构（新建任务、搜索、自动化、插件市场；带空态文案的项目/任务分区；底部账号簇占位），仍声明相同子 seat，并把新建任务接到 `startSession`。Web 组合保持关闭该标志并沿用 `SidebarRoot`。Desktop 组合在 ui-sidebar 的 loader entry 上设置 `config.desktopRail: true`（或由未来 desktop slot 注入同一标志）。

实时收起时，Web 外壳会把展开内容固定在当前宽度，并用 150ms 将其淡出。随后，上方四个控件——外壳的侧栏切换与新建会话，以及通过 `sidebar.workspaces` 渲染的添加和搜索——共用一次 150ms 的淡入和 49px 左移，在布局的 300ms 栏滑动结束时一起进入 56px 轨道；每个 36px 控件盒都会沿同一条路径到达轨道左侧 10px 的内边距。固定在底部的 `sidebar.settings` 控件只共用淡入时序，不发生横向位移。页面初始即为收起状态时会静态渲染轨道；减少动态效果模式会禁用两段过渡。DesktopRail 使用相同栏宽与收起图标外壳，但不走 Web 的交叉淡入状态机。
...
页脚承载 `sidebar.settings`：侧边栏只渲染固定在底部的布局 slot，并共享其栏状态（`wide`）；ui-settings 在此注册触发行和设置面板。DesktopRail 在该 seat 上方叠放账号占位。

`/client` 导出表层只包含插件主体（`apply`／`inject`／`Config`）及约定类型；SidebarRoot、DesktopRail、行组件和树派生仍由 slot 注册封装在包内。
...
- **DesktopRail 上除新建任务外的动作仍是呈现层**：搜索、自动化、插件市场与账号占位带有稳定 id/action，供后续路由接入；当前不导航。
