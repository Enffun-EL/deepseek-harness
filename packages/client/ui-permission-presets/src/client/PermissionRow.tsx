...
import { FULL_ACCESS_PRESET, translatePermissionPreset } from './presentation.ts'
...
export function PermissionRow({ load, select, usePermission, t }: PermissionRowProps) {
  ...
  const optionLabel = (id: string, name: string): string => translatePermissionPreset(id, name, t)
  const label = selected !== undefined
    ? optionLabel(selected.id, selected.name)
      ...
      <div className={css.row}>
        ...
        <Menu
          ...
          items={state.options.map(option => ({
            id: option.id,
            label: optionLabel(option.id, option.name),
          }))}
