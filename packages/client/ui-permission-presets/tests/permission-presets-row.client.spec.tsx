...
describe('PermissionRow', () => {
  it('loads the descriptor, opens the menu, and selects a new default', async () => {
    ...
    mount(controller)
    const button = await screen.findByRole('button', { name: 'Ask before edits' })
    ...
    expect(button.getAttribute('aria-expanded')).toBe('false')
    fireEvent.click(button)
    fireEvent.click(screen.getByRole('menuitem', { name: 'Ask before edits' }))
    expect(mutate).not.toHaveBeenCalled()
    fireEvent.click(button)
    fireEvent.click(screen.getByRole('menuitem', { name: 'Auto edit' }))
    await screen.findByRole('button', { name: 'Auto edit' })
  ...
  it('requires explicit acknowledgement before saving Full access', async () => {
    ...
    mount(controller)
    fireEvent.click(await screen.findByRole('button', { name: 'Ask before edits' }))
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Ask before edits' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Full access' }))
  ...
  it('hides an unavailable namespace and disables a read-only provider', async () => {
    ...
    mount(readonly)
    expect((await screen.findByRole('button', { name: 'Ask before edits' })).hasAttribute('disabled')).toBe(true)
  })
