...
describe('ui-permission browser plugin', () => {
  ...
  it('availability follows the projection key; options mark the current value active and exclude custom', async () => {
    ...
    // Conventional presets use product labels; non-conventional names pass through.
    expect(again.map(option => option.label)).toEqual(['Ask before edits', 'Auto edit', 'Full access'])
